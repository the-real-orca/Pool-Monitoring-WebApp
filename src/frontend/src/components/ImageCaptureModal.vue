  ...
  try {
    ...
    if (result) {
      if (result.ph === -1 || result.cl === -1) {
        ...
        localError.value = `KI konnte nicht zuverlässig lesen: ${parts.join(', ')}`
        return
      }
      ...
      localError.value = 'Nicht autorisiert – Token prüfen'
    } else if (error.value === '422') {
      localError.value = 'KI konnte das Bild nicht analysieren'
    } else if (error.value === '429') {
      localError.value = 'Tägliches Bildanalyse-Limit erreicht'
    } else if (error.value) {
      localError.value = `Fehler ${error.value}`
    } else {
      localError.value = 'Netzwerkfehler'
    }
  } catch {
    localError.value = 'Bilddatei konnte nicht gelesen werden'
  }
}
</script>
